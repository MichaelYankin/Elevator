...
static void parseCommand(std::string cmd, int& cmdNumber, int& floor, int maxFloors) {

    /* Вызов лифта на этаже или запрос на этаж */
    if (cmd[0] == 'f' || cmd[0] == 'F' || cmd[0] == 'c' || cmd[0] == 'C') {
        if (cmd.size() == 2) {
            /* Проверяем, является ли введенная команда числом */
            if (int(cmd[1]) < 48 || int(cmd[1]) > 58) {
                /* Если нет - считаем, что получена неизвестная команда */
                cout << "Invalid Command.\n";
                cmdNumber = 4;
            }
            else {
                /* Преобразование в номер этажа */
                int tmp_floor = int(cmd[1]) - 48;
                /* Проверяем корректность введенного числа */
                if (tmp_floor > 0 && tmp_floor <= maxFloors) {
                    if (cmd[0] == 'f' || cmd[0] == 'F')
                        /* вызов на этаже */
                        cmdNumber = 0;
                    else
        ...
        else
        {
            ...
            if (int(cmd[1]) < 48 || int(cmd[1]) > 58 || int(cmd[2]) < 48 || int(cmd[2]) > 58) {
                ...
                /* Преобразование в номер этажа */
                int tmp_floor = 10 * (int(cmd[1]) - 48) + int(cmd[2]) - 48; 
                /* Проверяем корректность введенного числа */
                if (tmp_floor > 0 && tmp_floor <= maxFloors) {
                    if (cmd[0] == 'f' || cmd[0] == 'F')
                        /* вызов на этаже */
...
bool cElevator::command(std::string cmd) {
    ...
    */
    ...
    if (cmd == "none")
        ...
        parseCommand(cmd, cmdNumber, newFloor, maxFloors);
